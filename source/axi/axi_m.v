// 1. 视频帧的DDR3存储与读取
// 写操作：将视频数据通过AXI总线写入DDR3（帧缓存）
// 读操作：从DDR3读取视频数据供输出显示

// 2. 双缓冲机制（Ping-Pong Buffer）
// 使用页地址切换实现帧缓存管理：
// 写入当前帧到页A
// 同时读取上一帧从页B
// 写完一帧后自动切换写页，避免读写冲突
// 地址构成：{4'b0, VIDEO_BASE_ADDR, 页地址(2bit), 行地址(20bit)}
//          ↑        ↑              ↑              ↑
//       保留位   视频基地址     帧缓存页       突发地址计数

// 3. FIFO水位控制
// 根据FIFO的水位（数据量）自动发起突发传输：
// 写触发：写FIFO水位 > 突发长度时，启动AXI写传输
// 读触发：读FIFO水位 < 阈值时，启动AXI读传输
